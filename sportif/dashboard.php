    <?php
        ...
        if(isset($_POST['cancel_seance'])) {
            $seance_id = $_POST['seance_id'];

            $find_stmt = mysqli_prepare($conn, '
            ...
            if($find_stmt) {
                mysqli_stmt_bind_param($find_stmt, 'ii', $seance_id, $user_id);
                mysqli_stmt_execute($find_stmt);
                $res = mysqli_stmt_get_result($find_stmt);
                $seance_data = mysqli_fetch_assoc($res);

                if($seance_data && !empty($seance_data['id_disponibilite'])) {
                    mysqli_begin_transaction($conn);

                    try {
                        $stmt = mysqli_prepare($conn, 'DELETE FROM seance WHERE id_seance = ?');
                        mysqli_stmt_bind_param($stmt, 'i', $seance_id);
                        mysqli_stmt_execute($stmt);

                        $upd_stmt = mysqli_prepare($conn, "UPDATE disponibilite SET statut = 'libre' WHERE id_disponibilite = ?");
                        mysqli_stmt_bind_param($upd_stmt, 'i', $seance_data['id_disponibilite']);
    ...
    <body class="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 min-h-screen">
        ...
        <div class="container mx-auto px-4 sm:px-6 py-6 sm:py-8 lg:py-10">
            ...
            <div class="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-xl p-4 sm:p-6 lg:p-8">
                ...
                <?php if (mysqli_num_rows($card_result) > 0): ?>
